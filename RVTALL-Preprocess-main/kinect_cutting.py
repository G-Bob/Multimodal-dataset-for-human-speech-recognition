...
if __name__ == '__main__':
    bvh_root = r'D:\gy\mouth data\Raw data\kinect'
    bvh_save = r'D:\gy\mouth data\processed&cut data\kinect_processed'
  
    error_rec = []
    
    
# Step 1: extract 3D positions from bvh files
    kinect_list = os.listdir(bvh_root)
    print(kinect_list) #kincet folder
    for f in tqdm.tqdm(kinect_list):
        bvhrootfolder =(bvh_root+r'\{list}'.format(list=f))#*_kinect folder        
        
        kinect_files = os.listdir(bvhrootfolder)
        
        
        print(r'root floder is: '+ bvhrootfolder)        
        
        
        for i in kinect_files:
            folders = (bvhrootfolder+r'\{index}'.format(index=i))#vowel word sentences folder
            landmarkerfolder =(folders+r'\landmarkers') 
            bvhsavefolder =(bvh_save+r'\{list}'.format(list=f)+r'\{index}'.format(index=i))
            print(r'processed folder is: '+ bvhsavefolder)
            
            if not os.path.exists(bvhsavefolder):
                os.makedirs(bvhsavefolder)
                
            bvh_processer = BVHReader(save_folder=landmarkerfolder) # the final folder must be named as 'landmarkers'
            print(r'content list is : '+folders)
            print(r'landmarkers folder is: '+landmarkerfolder)
            
            files = os.listdir(folders)
            print(files)
            for k in files:
                bvhfiles = (folders+r'\{names}'.format(names=k)) 
                                
                if bvhfiles ==(folders+r'\kinect_bvh') :   
                    print(r'Now working on: '+bvhfiles) 
                    try:
                        bvh_processer.proc_folder(tgt_folder=bvhfiles) # this folder is 'kinect_bvh' folder in the raw collection
                    except :
                        info = traceback.format_exc()
                        error_rec.append(info)
                        error_rec.append(bvhfiles)
                        print(info)
                # Step 2: process 3D position data, video and audio
            kinect = KinectProcessor(root_dir=folders, save_dir=bvhsavefolder)
            try:
                kinect.extract_frames_folder()
            except:
                err = traceback.format_exc()
                error_rec.append(err)
                error_rec.append(folders)
                
name = 'err_rec_1-20.csv'
with open(name,'w')as f:
    csvwriter = csv.writer(f)
    csvwriter.writerows(error_rec)
